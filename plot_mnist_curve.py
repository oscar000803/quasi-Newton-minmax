...
epochs = 60
step = 2
pattern = "./checkpoints/mnist/{}-{}-{}-{}-{}/{}-epoch_{:d}.tar"

def comp_gen2(g_optim, g_stepsize, d_optim, d_stepsize, d_num):
    dist = np.array([0.]*int(epochs/step))
    for i in range(step, epochs + step, step):
        dist0 = torch.load(pattern.format(g_optim, g_stepsize, d_optim, d_stepsize, d_num, "generator", i), map_location='cpu')['gradient']
        dist[int(i/step) - 1] = norm(dist0)
    return dist

def comp_dis(g_optim, g_stepsize, d_optim, d_stepsize, d_num):
    dist = np.array([0.]*int(epochs/step))
    for i in range(step, epochs + step, step):
        dist0 = torch.load(pattern.format(g_optim, g_stepsize, d_optim, d_stepsize, d_num, "discriminator", i), map_location='cpu')['gradient']
        dist[int(i/step) - 1] = norm(dist0)
    return dist
...
if __name__ == "__main__":
    # epochs = 90
    interval = step
    # dist1 = comp_gen2('newton', 1.0, 'newton', 1.0, 1)[:int(epochs/interval)]
    # print('generator', dist1)
    # np.save(g_optim + d_optim + '_gen.npy', dist1)
    dist2 = comp_gen2('LBFGS', 0.01, 'LBFGS', 0.5, 20)[:int(epochs/interval)]
    #dist3 = comp_gen2('sd', 'gd')[:int(epochs/interval)]
    #dist4 = comp_gen2('gd', 'fr')[:int(epochs/interval)]

    # ddist1 = comp_dis('newton', 1.0, 'newton', 1.0, 1)[:int(epochs/interval)]
    ddist2 = comp_dis('LBFGS', 0.01, 'LBFGS', 0.5, 20)[:int(epochs/interval)]
    ...
    ax1 = plt.subplot(121)
    # ax1.plot(range(0, epochs, interval), dist1, label='cn', linestyle='-')
    ax1.plot(range(0, epochs, interval), dist2, label='clbfgs', linestyle='--')
    ...
    ax2 = plt.subplot(122)
    # ax2.plot(range(0, epochs, interval), ddist1, label='cn', linestyle='-')
    ax2.plot(range(0, epochs, interval), ddist2, label='clbfgs', linestyle='--')
